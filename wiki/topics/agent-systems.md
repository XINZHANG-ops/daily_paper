...
Agent systems research is rapidly evolving beyond static, single-deployment paradigms toward architectures that support continuous learning, collective improvement, and real-world deployment at scale. The four papers in this topic define the emerging frontiers: world models for internal simulation and long-term memory, collective skill evolution across multi-user deployments, trustworthy evaluation frameworks, and real-world web benchmarks.

OpenWorldLib establishes the foundational definition that distinguishes true world models from mere video generation systems, emphasizing three core capabilities: perception of the physical world, action-conditioned simulation, and long-term memory. This framework positions world models as something fundamentally different from tools like Sora that generate videos but lack interactive capabilities. The five-core architecture (Operator, Synthesis, Reasoning, Representation, Memory) provides a unified scaffold for integrating diverse agent capabilities, drawing connections to embodied foundation models like HY-Embodied-0.5 that fit within this definition.
